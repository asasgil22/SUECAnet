...
export default function Home() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErroAuth(err.message || "Erro de conexão com o servidor.");
    }
    setLoading(false);
  };
  ...
  const iniciarJogo = (salaId: string, buyIn: number) => {
    if (
      usuarioLogado &&
      usuarioLogado.moedas < buyIn &&
      salaId !== "TREINO-LOCAL"
    ) {
      alert(
        `Saldo insuficiente! Você precisa de ${buyIn} SuecaCoins para entrar na sala.`,
      );
      return;
    }
  ...
  return (
    <div className="min-h-[100dvh] bg-[#050a07] text-white flex items-center justify-center relative overflow-x-hidden font-sans p-4 sm:p-6 pb-12">
      ...
      <AnimatePresence mode="wait">
        {bonusAlerta && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed top-2 sm:top-6 z-50 bg-emerald-500 text-black font-bold px-4 sm:px-6 py-2 sm:py-3 rounded-full shadow-lg text-sm sm:text-base text-center w-[90%] sm:w-auto left-1/2 -translate-x-1/2"
          >
      ...
      <AnimatePresence mode="wait">
        {!usuarioLogado ? (
          <motion.div
            ...
            className="relative z-10 w-full max-w-md bg-gradient-to-br from-black/60 via-black/50 to-black/70 backdrop-blur-3xl border border-emerald-500/40 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl mt-4 sm:mt-0"
          >
            <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/5 via-transparent to-emerald-600/5 pointer-events-none rounded-[2rem] sm:rounded-[2.5rem]"></div>
            <div className="flex justify-center mb-4 sm:mb-6 text-5xl sm:text-6xl drop-shadow-[0_0_20px_rgba(52,211,153,0.8)] animate-pulse relative z-10">
              ♠️
            </div>

            <h1 className="text-3xl sm:text-4xl font-black text-center text-white tracking-widest mb-2 relative z-10">
              SUECA
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-emerald-300 to-emerald-600 drop-shadow-[0_0_10px_rgba(52,211,153,0.5)]">
                bet
              </span>
            </h1>
            ...
            {/* O "text-base" nos inputs evita o auto-zoom em iPhones ao clicar nos campos */}
            <form
              onSubmit={handleAuth}
            ...
            >
              {!isLogin && (
                <>
                  <input
                    ...
                    className="text-base bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-emerald-500 focus:bg-black/50 transition-all w-full"
                  />
                  <div className="relative w-full">
                    <input
                      ...
                      className="text-base w-full bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-emerald-500 focus:bg-black/50 transition-all"
                    />
                    <div className="mt-2 bg-gradient-to-br from-rose-500/15 to-rose-600/10 border border-rose-500/40 p-2 sm:p-3 rounded-xl flex items-start gap-2 sm:gap-3 w-full">
                      ...
                      <p className="text-[9px] sm:text-[10px] text-rose-200/90 leading-relaxed font-semibold tracking-tight">
                        Jogos de azar são estritamente proibidos para menores de
                        18 anos. CPF obrigatório.
                      </p>
                    </div>
                  </div>
                  <input
              ...
              {erroAuth && (
                <div className="bg-gradient-to-r from-rose-500/20 to-red-500/20 border border-rose-500/50 p-3 rounded-lg text-rose-300 text-xs text-center font-bold animate-pulse mt-1">
                  ❌ {erroAuth}
                </div>
              )}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-emerald-500 to-emerald-400 text-black font-black py-3 sm:py-4 rounded-xl mt-2 sm:mt-4 hover:opacity-90 active:scale-95 shadow-lg transition-all uppercase tracking-widest text-sm sm:text-base disabled:opacity-50 disabled:active:scale-100"
              >
                {loading
                  ? "⏳ Processando..."
                  : isLogin
            ...
            <div className="relative z-20">
              ...
              <button
                ...
                className="w-full text-center text-[10px] sm:text-xs text-white/40 hover:text-emerald-300 font-bold uppercase tracking-wider py-2 transition-colors"
              >
                {isLogin
          ...
          >
            {/* Header Mobile First: Adaptável ao invés de fixo/absoluto */}
            <div className="w-full flex flex-col sm:flex-row items-center justify-between gap-4 mb-6 sm:mb-12 bg-black/30 sm:bg-transparent p-4 sm:p-0 rounded-2xl border border-white/5 sm:border-none">
              <div className="hidden sm:block text-2xl drop-shadow-[0_0_10px_rgba(52,211,153,0.8)] animate-pulse">
                ♠️
              </div>
              <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 w-full sm:w-auto">
                {usuarioLogado.moedas === 0 && (
                  <button
                    ...
                    className="animate-pulse bg-rose-600 text-white text-[10px] sm:text-xs font-bold px-3 py-1.5 sm:px-4 sm:py-2 rounded-full shadow-[0_0_10px_rgba(225,29,72,0.5)]"
                  >
                    🆘 Resgate
                  </button>
                )}
                <div className="px-3 sm:px-5 py-1.5 sm:py-2 bg-gradient-to-r from-yellow-500/20 to-amber-600/20 border border-yellow-500/40 rounded-full flex items-center gap-1.5">
                  ...
                  <span className="text-white/60 text-[9px] sm:text-xs font-medium uppercase tracking-widest hidden sm:inline">
                    SuecaCoins
                  </span>
                </div>
                <div className="px-3 sm:px-4 py-1.5 sm:py-2 bg-gradient-to-r from-emerald-500/10 to-blue-500/10 border border-emerald-500/30 rounded-full backdrop-blur-sm">
                  <span className="text-white/60 text-[10px] sm:text-sm font-medium">
                    👤{" "}
                ...
                <button
                  onClick={logout}
                  className="text-white/40 hover:text-rose-300 text-[10px] sm:text-xs font-bold uppercase tracking-widest border border-white/10 px-3 py-1.5 rounded-full hover:bg-rose-500/10 hover:border-rose-500/30 active:scale-95 transition-all"
                >
                  Sair
                </button>
              </div>
            ...
            <div className="flex flex-col lg:flex-row items-center sm:items-start justify-between gap-10 lg:gap-16 w-full">
              {/* Coluna Esquerda: Título e Salas */}
              <div className="flex-1 flex flex-col items-center lg:items-start w-full text-center lg:text-left">
                <div className="mb-2">
                  <p className="text-emerald-400 text-[9px] sm:text-xs font-bold tracking-[0.3em] sm:tracking-[0.4em] uppercase drop-shadow-[0_0_10px_rgba(52,211,153,0.4)] animate-pulse">
                    ♠️ A mesa está pronta ♠️
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full px-2 sm:px-0">
                  {CATEGORIAS_SALAS.map((sala) => (
                    <button
                      ...
                      className={`relative group overflow-hidden bg-black/40 border border-white/10 p-5 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] flex flex-col items-center hover:-translate-y-1 transition-all duration-300 active:scale-95`}
                    >
                      <div
                        className={`absolute inset-0 bg-gradient-to-b ${sala.cor} opacity-10 sm:group-hover:opacity-30 transition-opacity duration-500`}
                      ></div>
                      <span className="text-4xl sm:text-5xl mb-2 sm:mb-3 relative z-10 drop-shadow-lg">
                        {sala.icone}
                      </span>
                      <h3 className="text-white font-black text-lg sm:text-xl mb-1 relative z-10 uppercase tracking-wider">
                        {sala.nome}
                      </h3>
                      <div className="flex items-center gap-1 bg-black/50 px-2 sm:px-3 py-1 rounded-full mb-3 sm:mb-4 relative z-10 border border-white/5">
                        <span className="text-[9px] sm:text-[10px] text-white/50 uppercase tracking-widest font-bold">
                          Buy-in:
                        </span>
                      ...
                      <p className="text-emerald-400/80 text-[10px] sm:text-xs font-bold uppercase tracking-widest relative z-10">
                        Pote: {sala.pot} SC
                      </p>
                    </button>
                  ))}
                ...
                <button
                  ...
                  className="mt-6 sm:mt-8 text-white/40 text-[10px] sm:text-xs font-bold uppercase tracking-widest hover:text-emerald-400 transition-colors flex items-center gap-2 mx-auto lg:mx-0"
                >
                  <span>🤖</span> Treinar Offline (Grátis)
                </button>
              </div>

              {/* Coluna Direita: Seleção de Baralho Responsiva */}
              <div className="w-full sm:w-[350px] lg:w-[400px] flex flex-col items-center mt-6 lg:mt-0 px-2 sm:px-0">
                <div className="bg-gradient-to-br from-[#0b1510]/90 to-black/80 backdrop-blur-2xl border border-emerald-500/20 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] w-full flex flex-col items-center shadow-xl">
                  <div className="mb-4 sm:mb-8 text-center">
                    <span className="text-[8px] sm:text-[9px] text-emerald-400/60 uppercase tracking-[0.3em] sm:tracking-[0.4em] font-bold">
                      ✨ Design do Baralho
                    </span>
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    <motion.div
                      ...
                      className={`relative w-48 sm:w-56 h-64 sm:h-72 rounded-[1.2rem] sm:rounded-[1.5rem] shadow-2xl border-2 border-emerald-500/40 bg-cover bg-center overflow-hidden flex items-center justify-center ${temaSelecionado.cor}`}
                    >
                      {temaSelecionado.escudo ? (
                        <img
                          src={temaSelecionado.escudo}
                          alt={temaSelecionado.sigla}
                          className="w-24 h-24 sm:w-28 sm:h-28 object-contain drop-shadow-2xl z-10 opacity-95"
                        />
                      ) : (
                        <span className="font-serif text-6xl sm:text-7xl font-black text-white/60 z-10 drop-shadow-lg">
                  ...
                  <div className="flex items-center justify-between w-full mt-6 sm:mt-10 relative z-10">
                    <button
                      onClick={prevTema}
                      className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white/5 hover:bg-emerald-500/20 flex items-center justify-center text-white/40 hover:text-emerald-300 transition-colors border border-white/10 active:scale-90"
                    >
                      ❮
                    </button>
                    <div className="text-center px-2 flex-1 overflow-hidden">
                      <p className="font-bold text-white text-sm sm:text-lg tracking-wide whitespace-nowrap text-ellipsis">
