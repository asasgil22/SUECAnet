...
const DICAS_SUECA = [
  "O Ás é a carta mais valiosa do jogo, rendendo 11 pontos para a sua dupla.",
  "A carta 7 (Manilha) é a segunda mais forte do naipe, valendo 10 pontos.",
  "Sempre acompanhe o naipe da primeira carta jogada na mesa. É obrigatório!",
  "Se não tiver o naipe da mesa, use um Trunfo para roubar a vaza dos adversários.",
  "Guarde seus Trunfos mais altos para capturar os Ases e Setes dos oponentes.",
  "Em uma vaza sem trunfos, a carta mais alta do naipe inicial sempre vence.",
  "Preste atenção nas cartas que já saíram. A memória é a sua maior arma.",
  "O Rei (K) vale 4 pontos, o Valete (J) vale 3 e a Dama (Q) vale apenas 2 pontos.",
  "Cartas numéricas de 2 a 6 não valem pontos, mas são ótimas para descartar.",
];
...
export default function GameBoard({
  ...
  const [infoJogadores, setInfoJogadores] = useState<
    Record<string, { nome: string; isBot: boolean; dupla: number }>
  >({});
  ...
  const playHaptic = (
    type: "vaza" | "vitoria" | "erro" | "vitoria-global" | "derrota-global",
  ) => {
  ...
  const idEsquerda =
    meuIndex !== -1 ? idsMesaSegura[(meuIndex + 1) % 4] : "bot-esquerda";
  const idFrente =
    meuIndex !== -1 ? idsMesaSegura[(meuIndex + 2) % 4] : "bot-frente";
  const idDireita =
    meuIndex !== -1 ? idsMesaSegura[(meuIndex + 3) % 4] : "bot-direita";
  ...
  useEffect(() => {
    ...
    novaConexao.on("connect", () => {
      ...
      novaConexao.emit("entrarSala", { salaId, tema, nome: nomeJogador });
    });
  ...
  const jogarCarta = (cartaJogada: Carta) => {
    ...
    if (naipePuxado && cartaJogada.naipe !== naipePuxado) {
      const temNaipe = mao.some((c) => c.naipe === naipePuxado);
      if (temNaipe) {
        playHaptic("erro");
        setAlertaJogada(
    ...
    >
      ...
      <div className="w-full flex flex-col items-center relative z-10 mt-4 mb-2">
        ...
        >
          <OpponentHand
            posicao="frente"
            quantidade={mao.length > 0 ? mao.length : 1}
            tema={temaFrenteObj}
          />
        </div>
      </div>
      ...
      <div className="w-full flex flex-col items-center relative z-30 mt-4">
