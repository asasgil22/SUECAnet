...
export default function GameBoard({
  ...
  const getPosicaoCartaMesa = (jogadorId: string, indexFallback: number) => {
    if (jogadorId === idAbaixo)
      return { y: "65px", x: "5px", rotate: -4, zIndex: 40 };
    if (jogadorId === idFrente)
      return { y: "-65px", x: "-5px", rotate: 4, zIndex: 20 };
    if (jogadorId === idEsquerda)
      return { x: "-95px", y: "10px", rotate: -22, zIndex: 10 };
    if (jogadorId === idDireita)
      return { x: "95px", y: "-10px", rotate: 22, zIndex: 30 };
    return {
  ...
  return (
    <div
      className="w-full h-full flex flex-col justify-between relative p-2 sm:p-6 pt-8 sm:pt-4 select-none font-sans overflow-hidden transition-colors duration-1000"
    ...
    >
      ...
      <div className="fixed bottom-40 sm:bottom-6 right-2 sm:right-6 z-[60] flex flex-col gap-2">
        <AnimatePresence>
          {emojisMensagens
        ...
        <div className="bg-[#0b1510]/95 backdrop-blur-xl border border-emerald-500/30 rounded-2xl p-2 grid grid-cols-5 gap-1 shadow-[0_0_20px_rgba(16,185,129,0.3)]">
          {EMOJIS_CLASH.map((emoji) => (
            <button
              ...
              className="text-lg sm:text-2xl p-1 hover:scale-125 active:scale-90 transition-transform cursor-pointer rounded hover:bg-emerald-500/10"
              title={`Enviar ${emoji}`}
            >
              {emoji}
            </button>
      ...
      <AnimatePresence>
        {alertaJogada && (
          <motion.div
            ...
            className="absolute bottom-52 sm:bottom-56 left-1/2 -translate-x-1/2 z-[60] bg-rose-600/95 backdrop-blur-xl border border-rose-400 px-4 py-2 rounded-[2rem] shadow-[0_15px_40px_rgba(225,29,72,0.6)] flex items-center gap-2"
          >
            <span className="text-lg animate-bounce">⚠️</span>
            <span className="text-white text-[11px] sm:text-sm font-black tracking-widest uppercase">
              {alertaJogada}
            </span>
          </motion.div>
        )}
        ...
        {aguardandoInicio && !isSpectator && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              className="bg-gradient-to-b from-[#0b1510] to-[#050a07] border border-emerald-500/30 p-6 sm:p-10 rounded-[3rem] text-center max-w-[28rem] w-full shadow-[0_0_100px_rgba(16,185,129,0.15)] flex flex-col items-center relative overflow-hidden"
            >
              ...
              <div className="relative w-36 h-36 sm:w-44 sm:h-44 flex items-center justify-center mb-6 z-10">
                ...
                <div className="flex flex-col items-center justify-center bg-black/30 w-24 h-24 sm:w-32 sm:h-32 rounded-full border border-white/5 backdrop-blur-md shadow-inner">
                  <span className="text-3xl sm:text-5xl font-black text-white font-mono tracking-tighter drop-shadow-md">
                    {tempoEsperaModal}
                  </span>
                ...
                <div className="w-full relative flex items-center justify-center min-h-[2.5rem]">
                  <AnimatePresence mode="wait">
                    <motion.p
                      ...
                      className="text-xs text-white/90 text-center leading-relaxed font-medium italic"
                    >
      ...
      <AnimatePresence>
        {resultadoFim && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className={`absolute inset-0 z-[70] flex items-center justify-center p-4 backdrop-blur-3xl transition-colors duration-1000 ${resultadoFim.fimDeJogoGlobal ? (isVencedorGlobal ? "bg-amber-950/80" : "bg-rose-950/90") : "bg-black/60"}`}
          >
            {resultadoFim.fimDeJogoGlobal ? (
              <motion.div
                ...
                className={`bg-[#050a07] border ${isVencedorGlobal ? "border-amber-500/40 shadow-[0_0_120px_rgba(251,191,36,0.3)]" : "border-rose-500/40 shadow-[0_0_120px_rgba(225,29,72,0.3)]"} p-6 sm:p-10 rounded-[3rem] text-center max-w-md w-full relative overflow-hidden`}
              >
                <div
                  className={`mx-auto w-16 h-16 sm:w-24 sm:h-24 rounded-full flex items-center justify-center mb-4 sm:mb-6 border ${isVencedorGlobal ? "bg-amber-500/10 border-amber-500/30" : "bg-rose-500/10 border-rose-500/30"}`}
                >
                ...
                <h2
                  className={`text-2xl sm:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r mb-2 tracking-tight ${isVencedorGlobal ? "from-amber-200 to-yellow-500" : "from-rose-300 to-red-600"}`}
                >
                  {isVencedorGlobal ? "VITÓRIA GLORIOSA" : "FIM DA LINHA"}
                </h2>
                <p className="text-xs sm:text-sm text-white/60 mb-6 font-medium px-2">
                  {isVencedorGlobal
                ...
                <div className="flex justify-between items-center bg-black/50 p-4 sm:p-6 rounded-[2rem] mb-6 border border-white/5 shadow-inner">
                  <div className="flex flex-col items-center w-1/3">
                    <span
                      className={`text-[9px] uppercase tracking-[0.2em] font-bold mb-1 ${isVencedorGlobal ? "text-amber-400" : "text-white/40"}`}
                    >
                      Nós
                    </span>
                    <span
                      className={`text-3xl sm:text-5xl font-black tracking-tighter ${isVencedorGlobal ? "text-amber-400" : "text-white/40"}`}
                    >
                      {meusSets}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-col items-center w-1/3">
                    <span
                      className={`text-[9px] uppercase tracking-[0.2em] font-bold mb-1 ${!isVencedorGlobal ? "text-rose-400" : "text-white/40"}`}
                    >
                      Eles
                    </span>
                    <span
                      className={`text-3xl sm:text-5xl font-black tracking-tighter ${!isVencedorGlobal ? "text-rose-400" : "text-white/40"}`}
                    >
                      {setsAdversario}
                    </span>
                  </div>
                ...
                <div className="flex flex-col gap-3">
                  {!isSpectator ? (
                    <button
                      ...
                      className={`w-full font-black tracking-wide py-3 sm:py-4 rounded-[1.5rem] transition-all flex items-center justify-center gap-2 text-xs sm:text-base ${isVencedorGlobal ? "bg-amber-500 hover:bg-amber-400 text-black" : "bg-rose-600 hover:bg-rose-500 text-white"}`}
                    >
                  ...
                  <button
                    ...
                    className="w-full bg-transparent border border-white/10 text-white/60 hover:text-white font-bold py-3 rounded-[1.5rem] text-xs transition-all"
                  >
                    🚪 Sair para o Lobby
                  </button>
                </div>
              ...
              <motion.div
                ...
                animate={{ scale: 1, y: 0 }}
                transition={{ type: "spring", damping: 25 }}
                className="bg-[#0b1510]/95 border border-emerald-500/30 p-8 rounded-[3rem] text-center max-w-sm w-full shadow-2xl"
              >
                <h2 className="text-2xl font-extrabold text-emerald-400 mb-2">
                  Fim do Set
                </h2>
                ...
                <div className="flex justify-between items-center bg-black/40 p-4 rounded-[2rem] mb-6 border border-white/5">
                  <div className="flex flex-col items-center">
                    <span className="text-[9px] text-emerald-400 font-bold mb-1">
                      Nós
                    </span>
                  ...
                  <div className="flex flex-col items-center">
                    <span className="text-[9px] text-rose-400 font-bold mb-1">
                      Eles
                    </span>
                    <span className="text-3xl font-light text-white">
                      {pontosAdversario}
                ...
                {!isSpectator ? (
                  <button
                    ...
                    className="w-full bg-emerald-500 text-black font-bold py-3 rounded-[1.5rem] text-xs uppercase tracking-wider"
                  >
                    Próximo Set
                  </button>
                ) : (
      ...
      <div className="flex flex-wrap justify-between items-center w-full relative z-20 gap-2">
        <motion.div
          ...
          animate={{ y: 0, opacity: 1 }}
          className="flex flex-wrap bg-[#0b1510]/95 backdrop-blur-3xl p-1.5 sm:p-2 rounded-[2rem] border border-white/10 shadow-xl items-center gap-2"
        >
          <div className="bg-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-[1.5rem] flex items-center gap-2 border-2 border-emerald-500/50 shadow-md">
            <div className="flex flex-col">
              <span className="text-[7px] uppercase tracking-widest text-zinc-600 font-black">
                Trunfo
              </span>
              <span className="font-black text-lg sm:text-xl text-black leading-none">
                {trunfo?.valor || "-"}
              </span>
            </div>
          ...
          <div className="bg-black/40 px-3 py-1.5 rounded-[1.5rem] flex flex-col items-center border border-white/5">
            <span className="text-[7px] uppercase tracking-widest text-emerald-400 font-black">
              Naipe
            </span>
          ...
          <div className="px-2.5 py-1 flex flex-col items-center border-l border-white/10">
            <span className="text-[7px] uppercase tracking-widest text-white/40 font-black">
              Set
            </span>
          ...
          <div className="bg-amber-500/10 px-3 py-1 rounded-[1.5rem] flex flex-col items-center border border-amber-500/20">
            <span className="text-[7px] uppercase tracking-widest text-amber-400 font-black">
              Global
            </span>
        ...
        <div className="bg-[#0b1510]/80 backdrop-blur-2xl px-3 py-1.5 rounded-xl border border-white/5 flex items-center gap-1.5 shadow-md">
          <span
            className={`w-2 h-2 rounded-full ${conectado ? "bg-emerald-400 animate-pulse" : "bg-rose-500"}`}
          ></span>
          <span className="text-[9px] font-black tracking-wider text-white/70 uppercase">
            {salaId}
          </span>
        </div>
      </div>
      ...
      <div className="w-full flex flex-col items-center relative z-10 mt-1 mb-1">
        <div className="flex items-center gap-1.5 mb-1 bg-black/50 px-2.5 py-0.5 rounded-full border border-white/10 backdrop-blur-md">
          <span
            className={`text-[8px] px-1.5 py-0.5 rounded-full font-bold ${infoFrente.isBot ? "bg-amber-500/20 text-amber-400" : "bg-blue-500/20 text-blue-400"}`}
          >
            {infoFrente.isBot ? "🤖" : "👤"}
          </span>
      ...
      <div className="flex-1 w-full flex items-center justify-between relative z-10 px-1 sm:px-6 my-1">
        <div
          className={`flex flex-col items-center z-20 ${turnoAtual === idEsquerda ? "drop-shadow-[0_0_15px_rgba(255,255,255,0.4)]" : ""}`}
        >
        ...
        <div
          className={`flex flex-col items-center z-20 ${turnoAtual === idDireita ? "drop-shadow-[0_0_15px_rgba(255,255,255,0.4)]" : ""}`}
        >
          <div className="flex flex-col items-center mb-1 bg-black/50 px-2 py-1 rounded-xl border border-white/10 backdrop-blur-md max-w-[75px]">
            <span className="text-white text-[9px] font-bold truncate">
              {infoDireita.nome}
      ...
      {isSpectator ? (
        <div className="w-full flex flex-col items-center justify-center relative z-30 mb-2 h-28">
          <motion.div
            ...
            className="bg-[#0b1510]/95 backdrop-blur-xl border border-blue-500/30 px-6 py-4 rounded-2xl shadow-lg text-center max-w-xs"
          >
            <span className="text-2xl mb-1 block animate-pulse">👀</span>
            <h3 className="text-blue-400 font-black text-xs uppercase tracking-widest mb-1">
              Modo Espectador
            </h3>
            <p className="text-white/60 text-[10px] leading-relaxed">
              Você está na fila de espera e entrará no próximo set.
            </p>
          </motion.div>
        </div>
      ) : (
        <div className="w-full flex flex-col items-center relative z-30 mb-1">
          <div className="mb-2 sm:mb-1 flex items-center gap-1.5 bg-black/40 px-3 py-1 rounded-full border border-white/10 backdrop-blur-md z-50">
            <span className="bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-[8px] px-2 py-0.5 rounded-full font-bold">
              Você
            </span>
            ...
            <AnimatePresence mode="wait">
              {turnoAtual === "calculando" ? (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="flex items-center gap-1.5 bg-black/70 backdrop-blur-xl px-4 py-1.5 rounded-full border border-white/5 shadow-md"
                >
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="flex items-center gap-1.5 bg-black/60 backdrop-blur-xl px-4 py-1.5 rounded-full border border-white/5 shadow-md"
                >
              ...
              {mao.map((carta, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="absolute bottom-0 cursor-pointer hover:-translate-y-4 sm:hover:-translate-y-6 transition-transform"
                  >
                    <Card
